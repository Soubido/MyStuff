...
namespace NewRhinoGold.BezelStudio
{
    public static class HeadBuilder
    {
        public static List<Brep> CreateHead(Curve gemCurve, Plane plane, HeadParameters p)
        {
            ...
            {
                for (int i = 0; i < p.ProngPositions.Count; i++)
                {
                    try
                    {
                        double t = p.ProngPositions[i];
                        while (t > 1.0) t -= 1.0;
                        while (t < 0.0) t += 1.0;

                        // NEU: Individuelle Rotation pro Prong
                        double individualRotation = 0.0;
                        if (p.ProngRotations != null && i < p.ProngRotations.Count)
                            individualRotation = p.ProngRotations[i];

                        var prong = CreateProng(baseCrv, t, plane, p, individualRotation);
        ...
        // --- PRONGS ---
        // CHANGE: Zusätzlicher Parameter individualRotationDeg für individuelle Prong-Rotation
        private static Brep CreateProng(Curve rail, double t, Plane gemPlane, HeadParameters p, double individualRotationDeg)
        {
            ...
            profiles.Add(CreateProfileCurve(axisCurve, axisCurve.Domain.Min, p.TopDiameter, gemPlane, p, p.TopProfileRotation + individualRotationDeg));

            axisCurve.ClosestPoint(ptMid, out double tMid);
            if (tMid > axisCurve.Domain.Min + 0.1 && tMid < axisCurve.Domain.Max - 0.1)
                profiles.Add(CreateProfileCurve(axisCurve, tMid, p.MidDiameter, gemPlane, p, p.MidProfileRotation + individualRotationDeg));

            profiles.Add(CreateProfileCurve(axisCurve, axisCurve.Domain.Max, p.BottomDiameter, gemPlane, p, p.BottomProfileRotation + individualRotationDeg));
