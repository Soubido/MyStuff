...
namespace NewRhinoGold.Dialog
{
    public class GemCreatorDlg : Form
    {
        ...
        // NEU: Etwas größere Schrift für Inputs, damit die Boxen wachsen
        ...
        public GemCreatorDlg()
        {
            ...
            // Breite angepasst für die größeren Boxen
        ...
        private Control BuildLayout()
        {
            ...
            var layout = new DynamicLayout
            {
                ...
                Spacing = new Size(5, 8) // Etwas mehr vertikaler Abstand (8px)
            };
            ...
            // --- FIX: UNIT MODE (Kein Abstand mehr) ---
            // Wir packen Label und Radios in EINEN Container.
            ...
            // Als einzelne Zeile hinzufügen -> Umgeht die Spaltenausrichtung des DynamicLayouts
        ...
        private TableRow CreateRow(string labelText, Control control)
        {
            return new TableRow(
                new TableCell(CreateLabel(labelText), false),
                new TableCell(control, true)
            );
        }

        private NumericStepper CreateNumeric(double val) => new NumericStepper
        {
            DecimalPlaces = 1,
            MinValue = 0,
            MaxValue = 999,
            Value = val,
            // FIX: Größere Schrift + Explizite Höhe (ca. 20% größer als Standard)
            Font = _fontInput,
            Height = 26
        };
        ...
        private void LoadCompounds()
        {
            ...
            foreach (var gem in Densities.Gems)
            {
                _comboCompound.Items.Add(gem.Name);
            }

            if (_comboCompound.Items.Count > 0)
            {
                _comboCompound.SelectedIndex = 0;
            }
        }
        ...
        {
            if (_comboCompound.SelectedIndex < 0) return null;
            return _comboCompound.Items[_comboCompound.SelectedIndex].Text;
        }

        private void OnPickColor(object sender, EventArgs e)
        {
            var dlg = new ColorDialog();
            dlg.Color = Color.FromArgb(_gemColor.R, _gemColor.G, _gemColor.B, _gemColor.A);

            if (dlg.ShowDialog(this) == DialogResult.Ok)
            {
                var c = dlg.Color;
                _gemColor = System.Drawing.Color.FromArgb((int)(c.A * 255), (int)(c.R * 255), (int)(c.G * 255), (int)(c.B * 255));
                Preview();
            }
        }
        ...
        {
            double size = _currentGemSizeVal;
            if (size < 0.001) return;

            _canUpdate = false;
            try
            {
                _numDInt.Value = _numDInt.Value * size / 100.0;
                _numH1.Value = _numH1.Value * size / 100.0;
                _numH2.Value = _numH2.Value * size / 100.0;
                _numH3.Value = _numH3.Value * size / 100.0;
            }
        ...
        private void OnSelectCurves(object sender, EventArgs e)
        {
            ...
            if (go.CommandResult() == Result.Success)
            {
                ...
                for (int i = 0; i < go.ObjectCount; i++)
                    _selectedCurveIds.Add(go.Object(i).ObjectId);

                if (doc.Views.ActiveView != null)
                    _workPlane = doc.Views.ActiveView.ActiveViewport.ConstructionPlane();
        ...
        private bool Preview()
        {
            ...
            _previewBreps.Clear();
            _previewPlanes.Clear();
            _previewBaseCurves.Clear();
            _previewCurves.Clear();

            double h1 = _numH1.Value;
            double h2 = _numH2.Value;
            double h3 = _numH3.Value;
            double dInt = _numDInt.Value;
            ...
            foreach (var id in _selectedCurveIds)
            {
                ...
                Curve baseCrv = obj.CurveGeometry ?? obj.Geometry as Curve;
                if (baseCrv == null) continue;

                var c2 = baseCrv.DuplicateCurve();
                Plane pl;
                if (!c2.TryGetPlane(out pl)) pl = _workPlane;
                if (_checkFlip.Checked == true) pl.Flip();

                _previewPlanes[index] = pl;
                _previewBaseCurves[index] = c2.DuplicateCurve();
                _previewCurves.Add(c2.DuplicateCurve());
                ...
                if (index == 0)
                {
                    _currentGemSizeVal = width;
                    _txtGemSize.Text = $"{width:F2} mm";
                }
        ...
        private void CalculateWeight(double volumeMm3)
        {
            ...
            if (string.IsNullOrEmpty(matName))
            {
                _txtWeight.Text = "0.00 ct";
                return;
            }

            double density = Densities.GetDensity(matName);
            double massGrams = volumeMm3 * (density / 1000.0);
            double carat = massGrams * 5.0;

            _txtWeight.Text = $"{carat:F2} ct";
        }

        private static Curve Point3dToCurve(Point3d pt)
        {
            return new Circle(new Plane(pt, Vector3d.ZAxis), 0.001).ToNurbsCurve();
        }
