    ...
    public sealed class MoveOnSurface : Command
    {
        ...
        protected override Result RunCommand(RhinoDoc doc, RunMode mode)
        {
            ...
            foreach (var r in go.Objects())
            {
                ...
                // FIX: GemSmartData auf Geometry.UserData suchen (primär),
                //      dann Fallback auf Attributes.UserData (Legacy)
                GemSmartData gemData = null;
                if (ro.Geometry != null)
                    gemData = ro.Geometry.UserData.Find(typeof(GemSmartData)) as GemSmartData;
                if (gemData == null)
                    gemData = ro.Attributes.UserData.Find(typeof(GemSmartData)) as GemSmartData;
        ...
        // ---------------------------------------------------------------
        // Getter mit Live-Vorschau
        // Strategie: Pro Frame transformierte Kopie zeichnen (kein PushModelTransform).
        // Farbe: Orange — sichtbar auf allen Oberflächen und Hintergründen in Rhino 8.
        // ---------------------------------------------------------------
        private class ObjectOnSurfaceGetter : GetPoint
        {
            ...
            {
                // Transformation auch hier berechnen — OnDynamicDraw kann
                // ohne vorheriges OnMouseMove aufgerufen werden
                ...
                for (int i = 0; i < _items.Count; i++)
                {
                    ...
                    // Transformierte Kopie erstellen und zeichnen
                    ...
                    {
                        // Rhino 8 speichert einfache Körper oft als Extrusion
                    ...
                    if (item.GapCurve != null)
                    {
                        ...
                        e.Display.DrawCurve(gap, GapColor, 2);
                    }
                }
